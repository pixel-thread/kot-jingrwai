...
When reviewing vault files or memory storage requests:
- **Missing YAML Metadata** — Ensure id, type, tags, and score are present.
- **Redundant Information** — Identify if the memory already exists in PRDs or the vault.
- **Vague Titles** — Ensure titles are descriptive and search-friendly.
- **Poor Chunking** — Ensure H2 headers are used properly for semantic segmentation.
